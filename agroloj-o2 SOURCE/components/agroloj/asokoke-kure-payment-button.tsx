...
interface AsoKokeKurePaymentButtonProps {
  onSuccess?: (result: any) => void
  onError?: (error: any) => void
  className?: string
}

export function AsoKokeKurePaymentButton({
  onSuccess,
  onError,
  className = "",
}: AsoKokeKurePaymentButtonProps) {
  const { sdk, products, restoredPurchases } = usePiAuth()
  const [isLoading, setIsLoading] = useState(false)
  const [error, setError] = useState<string | null>(null)

  // Get the Aso-Oke Kure product
  const product = products?.find(
    (p) => p.id === PRODUCT_CONFIG.PRODUCT_6a44f1a0fca142314b05d5eb
  )
