...
interface SoybeanPaymentButtonProps {
  onSuccess?: (result: any) => void
  onError?: (error: any) => void
  className?: string
}

export function SoybeanPaymentButton({
  onSuccess,
  onError,
  className = "",
}: SoybeanPaymentButtonProps) {
  const { sdk, products, restoredPurchases } = usePiAuth()
  const [isLoading, setIsLoading] = useState(false)
  const [error, setError] = useState<string | null>(null)

  // Get the Soybean product
  const product = products?.find(
    (p) => p.id === PRODUCT_CONFIG.PRODUCT_6a443bf0349aabbf10ae40f6
  )
