...
interface CatfishFarmingTrainingPaymentButtonProps {
...
export function CatfishFarmingTrainingPaymentButton({
  onSuccess,
  onError,
  className = "",
  variant = "full",
}: CatfishFarmingTrainingPaymentButtonProps) {
  ...
  const product = products?.find(
    (p) => p.id === PRODUCT_CONFIG.PRODUCT_6a450724bca67550aa7e8c1b
  )
    ...
    try {
      ...
      if (result.ok) {
        addBooking({
          ...
          serviceName: "Catfish Farming Training",
      ...
      <div className="flex flex-col gap-3 rounded-xl bg-muted/50 p-3 max-h-[80vh] overflow-y-auto">
        ...
        <div className="flex flex-col gap-2 border-t border-border pt-3 mt-2">
          <div className="flex items-center justify-between text-xs">
            ...
            <span className="font-semibold text-foreground">Catfish Farming Training</span>
          </div>
          <div className="flex items-center justify-between text-xs">
