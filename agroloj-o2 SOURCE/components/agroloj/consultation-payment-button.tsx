...
import { Loader2, Book, Calendar, Clock, User, Phone, Mail, AlertCircle } from "lucide-react"

interface ConsultationPaymentButtonProps {
...
export function ConsultationPaymentButton({
  ...
  className = "",
  variant = "full",
}: ConsultationPaymentButtonProps) {
  ...
  const product = products?.find(
    (p) => p.id === PRODUCT_CONFIG.PRODUCT_6a4433c5a7e60d0a32dad95a
  )
    ...
    try {
      ...
      if (result.ok) {
        ...
        addBooking({
          ...
          serviceName: "Consultancy Service",
      ...
      <div className="flex flex-col gap-3 rounded-xl bg-muted/50 p-3 max-h-[80vh] overflow-y-auto">
        ...
        <div className="flex flex-col gap-2 border-t border-border pt-3 mt-2">
          <div className="flex items-center justify-between text-xs">
            ...
            <span className="font-semibold text-foreground">Consultancy Service</span>
          </div>
          <div className="flex items-center justify-between text-xs">
