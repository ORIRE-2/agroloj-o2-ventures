...
import { Loader2, Egg, Calendar, Clock, User, Phone, Mail, AlertCircle } from "lucide-react"

interface PoultryConsultationPaymentButtonProps {
...
export function PoultryConsultationPaymentButton({
  onSuccess,
  onError,
  className = "",
  variant = "full",
}: PoultryConsultationPaymentButtonProps) {
  ...
  const product = products?.find(
    (p) => p.id === PRODUCT_CONFIG.PRODUCT_6a4653f2c7807267bdeb8090
  )
    ...
    try {
      ...
      if (result.ok) {
        addBooking({
          ...
          serviceName: "Poultry Consultation",
      ...
      <div className="flex flex-col gap-3 rounded-xl bg-muted/50 p-3 max-h-[80vh] overflow-y-auto">
        ...
        <div className="flex flex-col gap-2 border-t border-border pt-3 mt-2">
          <div className="flex items-center justify-between text-xs">
            ...
            <span className="font-semibold text-foreground">Poultry Consultation</span>
          </div>
          <div className="flex items-center justify-between text-xs">
