...
import { Loader2, Rabbit, Calendar, Clock } from "lucide-react"

interface RabbitryTrainingPaymentButtonProps {
...
export function RabbitryTrainingPaymentButton({
  onSuccess,
  onError,
  className = "",
  variant = "full",
}: RabbitryTrainingPaymentButtonProps) {
  ...
  const product = products?.find(
    (p) => p.id === PRODUCT_CONFIG.PRODUCT_6a44e1ef459c19f71138b690
  )
      ...
      <div className="flex flex-col gap-3 rounded-xl bg-muted/50 p-3">
        ...
        <div className="flex flex-col gap-2 border-t border-border pt-3 mt-2">
          <div className="flex items-center justify-between text-xs">
            ...
            <span className="font-semibold text-foreground">Rabbitry Training</span>
          </div>
          <div className="flex items-center justify-between text-xs">
