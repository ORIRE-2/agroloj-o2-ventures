...
interface AsoKokeEleyaPaymentButtonProps {
  onSuccess?: (result: any) => void
  onError?: (error: any) => void
  className?: string
}

export function AsoKokeEleyaPaymentButton({
  onSuccess,
  onError,
  className = "",
}: AsoKokeEleyaPaymentButtonProps) {
  const { sdk, products, restoredPurchases } = usePiAuth()
  const [isLoading, setIsLoading] = useState(false)
  const [error, setError] = useState<string | null>(null)

  // Get the Aso-Oke Eleya product
  const product = products?.find(
    (p) => p.id === PRODUCT_CONFIG.PRODUCT_6a44f2920ab5d51d1e5866c9
  )
