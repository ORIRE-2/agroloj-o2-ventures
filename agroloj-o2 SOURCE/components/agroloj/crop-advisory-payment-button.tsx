...
import { Loader2, Leaf, Calendar, Clock, User, Phone, Mail, AlertCircle } from "lucide-react"

interface CropAdvisoryPaymentButtonProps {
...
export function CropAdvisoryPaymentButton({
  onSuccess,
  onError,
  className = "",
  variant = "full",
}: CropAdvisoryPaymentButtonProps) {
  ...
  const product = products?.find(
    (p) => p.id === PRODUCT_CONFIG.PRODUCT_6a44fe799dbc068c1c3a126e
  )
    ...
    try {
      ...
      if (result.ok) {
        ...
        addBooking({
          ...
          serviceName: "Crop & Livestock Advisory",
      ...
      <div className="flex flex-col gap-3 rounded-xl bg-muted/50 p-3 max-h-[80vh] overflow-y-auto">
        ...
        <div className="flex flex-col gap-2 border-t border-border pt-3 mt-2">
          <div className="flex items-center justify-between text-xs">
            ...
            <span className="font-semibold text-foreground">Crop & Livestock Advisory</span>
          </div>
          <div className="flex items-center justify-between text-xs">
