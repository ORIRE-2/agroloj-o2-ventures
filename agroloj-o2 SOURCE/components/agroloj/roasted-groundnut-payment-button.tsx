...
interface RoastedGroundnutPaymentButtonProps {
...
export function RoastedGroundnutPaymentButton({
  onSuccess,
  onError,
  className = "",
  quantity = 1,
}: RoastedGroundnutPaymentButtonProps) {
  const { sdk, products } = usePiAuth()
  const [isLoading, setIsLoading] = useState(false)
  const [error, setError] = useState<string | null>(null)

  const product = products?.find(
    (p) => p.id === PRODUCT_CONFIG.PRODUCT_6a44e61bd69d96f95fdda6e7
  )
      ...
      >
        {isLoading ? (
          ...
          <>
            ...
            {product.price_in_pi && <span>· {product.price_in_pi} Pi</span>}
          </>
        )}
      </button>
      {error && (
