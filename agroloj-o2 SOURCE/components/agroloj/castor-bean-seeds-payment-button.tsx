...
interface CastorBeanSeedsPaymentButtonProps {
  onSuccess?: (result: any) => void
  onError?: (error: any) => void
  className?: string
}

export function CastorBeanSeedsPaymentButton({
  onSuccess,
  onError,
  className = "",
}: CastorBeanSeedsPaymentButtonProps) {
  const { sdk, products, restoredPurchases } = usePiAuth()
  const [isLoading, setIsLoading] = useState(false)
  const [error, setError] = useState<string | null>(null)

  // Get the Castor bean seeds product
  const product = products?.find(
    (p) => p.id === PRODUCT_CONFIG.PRODUCT_6a44ec98c9c746f9e46ea40d
  )
