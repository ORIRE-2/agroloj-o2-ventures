...
import { Loader2, Sprout, Calendar, Clock, User, Phone, Mail, AlertCircle } from "lucide-react"

interface FarmSetupPaymentButtonProps {
...
export function FarmSetupPaymentButton({
  onSuccess,
  onError,
  className = "",
  variant = "full",
}: FarmSetupPaymentButtonProps) {
  ...
  const product = products?.find(
    (p) => p.id === PRODUCT_CONFIG.PRODUCT_6a4652ef786e15f3e6d71df9
  )
    ...
    try {
      ...
      if (result.ok) {
        ...
        addBooking({
          ...
          serviceName: "Farm Setup Consultancy",
      ...
      <div className="flex flex-col gap-3 rounded-xl bg-muted/50 p-3">
        ...
        <div className="flex flex-col gap-2 border-t border-border pt-3 mt-2">
          <div className="flex items-center justify-between text-xs">
            ...
            <span className="font-semibold text-foreground">Farm Setup Consultancy</span>
          </div>
          <div className="flex items-center justify-between text-xs">
