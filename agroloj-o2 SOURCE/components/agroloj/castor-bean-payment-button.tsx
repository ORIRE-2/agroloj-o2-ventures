...
interface CastorBeanPaymentButtonProps {
  onSuccess?: (result: any) => void
  onError?: (error: any) => void
  className?: string
}

export function CastorBeanPaymentButton({
  onSuccess,
  onError,
  className = "",
}: CastorBeanPaymentButtonProps) {
  const { sdk, products, restoredPurchases } = usePiAuth()
  const [isLoading, setIsLoading] = useState(false)
  const [error, setError] = useState<string | null>(null)

  // Get the Castor bean seed product
  const product = products?.find(
    (p) => p.id === PRODUCT_CONFIG.PRODUCT_6a443f3683ba5aef9818de00
  )
